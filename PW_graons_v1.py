...
for kg in range(ng - 1):
    Us0[sl, kg] = Vs0[kg]
    if kg == 0:
        Us0[pqpv, kg] = vector_s0((U[:, pqpv_]), s0[kg])  # emplenar la tensió dels busos incògnita
        if npv > 0:
            Qs0[pv, kg] = vector_s0((Q[:, pv_]), s0[kg])  # emplenar la reactiva dels busos PV
    else:
        Us0[pqpv, kg] = vector_s0((Up[:, pqpv_, kg - 1]), s0[kg])
        if npv > 0:
            Qs0[pv, kg] = vector_s0((Qp[:, pv_, kg - 1]), s0[kg])
